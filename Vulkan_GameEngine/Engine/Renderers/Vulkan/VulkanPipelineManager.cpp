...
void VulkanPipelineManager::CreateGraphicsPipelines()
{
    ...
    for (const auto& shader : Manager->GetRenderData()->UIMapByShader)
    {
        ...
        if (M_UI_VulkanMaterial* vulkanMaterial = dynamic_cast<M_UI_VulkanMaterial*>(material)) CreateUIPipelineFromVulkanMaterial(shader.first, vulkanMaterial);
        else CreateUIPipelineFromGenericMaterial(shader.first, material);
    }
}
...
void VulkanPipelineManager::CreateUIPipelineFromGenericMaterial(std::string shader, M_UI_Material* material)
{
    //Create Shader Modules and Info
    std::vector<VkShaderModule> shaderModules;
    std::vector<VkPipelineShaderStageCreateInfo> shaderStages;
    ...
    pipelineLayoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
    pipelineLayoutInfo.setLayoutCount = 1;
    VkDescriptorSetLayout temp = Manager->GetUIDescriptorSetLayoutMap()[shader];
    pipelineLayoutInfo.pSetLayouts = &temp;
    pipelineLayoutInfo.pushConstantRangeCount = 0; // Optional
    pipelineLayoutInfo.pPushConstantRanges = nullptr; // Optional

    if (vkCreatePipelineLayout(Manager->GetLogicalDevice(), &pipelineLayoutInfo, nullptr, &UIPipelineMap[shader].second) != VK_SUCCESS)
    {
        throw std::runtime_error("failed to create pipeline layout!");
    }
    //-------------------------------------------------------------------------------
    ...
    pipelineInfo.pColorBlendState = &colorBlending;
    pipelineInfo.pDynamicState = &dynamicState;
    pipelineInfo.layout = UIPipelineMap[shader].second;
    pipelineInfo.renderPass = Manager->GetRenderPass();
    pipelineInfo.subpass = 0;
    pipelineInfo.basePipelineHandle = VK_NULL_HANDLE; // Optional
    pipelineInfo.basePipelineIndex = -1; // Optional

    if (vkCreateGraphicsPipelines(Manager->GetLogicalDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &UIPipelineMap[shader].first) != VK_SUCCESS)
    {
        throw std::runtime_error("failed to create graphics pipeline!");
    }
...
void VulkanPipelineManager::CreateUIPipelineFromVulkanMaterial(std::string shader, M_UI_VulkanMaterial* material)
{
